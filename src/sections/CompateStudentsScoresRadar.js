...
const CompateStudentsScoresRadar = ({
  data,
  options = {
    width: 500,
    height: 500,
  ...
  const draw = useCallback(() => {
    ...
    const selectedStudentsArr = [selectedStudents.a, selectedStudents.b]
    const features = [
      {
    ...
    const color = d3.scaleOrdinal(
      selectedStudentsArr.map((d) => d.code),
      options.colorScheme
    )
     ...
     * ------------------------------------------------
     */
    d3.select("#compareScoresRadarContainer svg").remove()
     ...
     */
    const svg = d3
      .select("#compareScoresRadarContainer")
      .append("svg")
    ...
    // Plotting the Data
    selectedStudentsArr.forEach((student, i) => {
  ...
  const handleStudentChange = useCallback(
    (e) => {
      setSelectedStudents((prev) => ({
        ...prev,
        [e.target.name]: data.find((d) => d.code.toString() === e.target.value),
      }))
    },
    [data]
  )
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="sm:text-center">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-lg leading-8 text-gray-600">
            See how two students are doing across their scores (e.g., Arabic
            language, college, current, etc.).
          </p>
        </div>

        <div
          id="compareScoresRadarContainer"
          className="mt-12 flex justify-center items-center flex-col gap-y-12"
        >
          <div className="flex justify-center items-center gap-6 flex-wrap">
            <Select
              id="scoresRadarStudentA"
              name="a"
              ...
              value={selectedStudents.a.code}
            />
            <Select
              id="scoresRadarStudentB"
              name="b"
              onChange={handleStudentChange}
              options={data.map((d) => ({
                label: `Student ${d.code}`,
...
CompateStudentsScoresRadar.propTypes = {
  data: PropTypes.array.isRequired,
}

export default CompateStudentsScoresRadar
